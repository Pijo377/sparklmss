...
export const SortableItem = ({ id, name, icon }: SortableItemProps) => {
    ...
    return (
        <div
            ...
            {...attributes}
            {...listeners}
            className="p-2.5 bg-white border border-slate-100 shadow-sm rounded-lg text-xs font-medium text-slate-700 cursor-move hover:border-blue-300 flex items-center justify-between mb-2 group active:scale-[0.98] transition-colors duration-200 touch-none select-none"
        >
...
export const DragPreview = ({ name, icon }: DragPreviewProps) => (
    <div className="p-2.5 bg-white border border-blue-200 shadow-lg rounded-lg text-xs font-medium text-slate-700 flex items-center justify-between group transition-none touch-none select-none w-full">
