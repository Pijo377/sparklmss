...
const mapPaydayFields = (form: any, sfx: string | number) => ({
  DayOfWeek: form['DayOfWeek' + sfx],
  DayOfWeek1: form['DayOfWeek1' + sfx],
  DayOfWeek2: form['DayOfWeek2' + sfx],
  PayWeek1: form['PayWeek1' + sfx],
  PayWeek2: form['PayWeek2' + sfx],
  SemiMonthlyPayDay01: form['SemiMonthlyPayDay01' + sfx],
  SemiMonthlyPayDay02: form['SemiMonthlyPayDay02' + sfx],
...
export const validateEmployer = (
  ...
  for (let i = 0; i < employerCount; i++) {
    ...
    if (!form['FirstPaymentDate' + sfx]) {
      ...
      // Must match nextPay or secondPay if they exist
      if (
        (nextPay && !isSameDay(firstPay, nextPay)) &&
        (secondPay && !isSameDay(firstPay, secondPay))
      ) {
...
export const EmployerInformation = ({
  ...
  const isAllowedWorkDay = (date: Date, index: number, sfx: string) => {
    ...
    // Check if current Monday is valid OR if the rule hit the previous Sat/Sun
    if (date.getDay() === 1) {
          ...
          >
            ...
            {form['SourceIncome' + sfx] === 'Employed' && (
              <>
                ...
                <Input label="Address 2" onChange={(e: any) => updateField('EmpAddress2' + sfx, e.target.value)} value={form.EmpAddress2} />
                <Input
                  label="State"
                  id={"EmployerState" + sfx}
                  error={errors['EmployerState' + sfx]}
                  value={form['EmployerState' + sfx]}
                  onChange={(e: any) => updateField('EmployerState' + sfx, e.target.value)}
                />
            ...
            <HolidayAwareDatePicker
              ...
              onValidDate={(date) => updateField('LastPayDate' + sfx, date)}
            />
            ...
            <HolidayAwareDatePicker
              ...
              onValidDate={(date) => updateField('NextPayDate' + sfx, date)}
            />
            ...
            <HolidayAwareDatePicker
              ...
              onValidDate={(date) => updateField('SecondPayDate' + sfx, date)}
            />
            ...
            <HolidayAwareDatePicker
              ...
              onValidDate={(date) => updateField('FirstPaymentDate' + sfx, date)}
            />
